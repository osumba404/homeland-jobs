...
async function main() {
  ...
  const jobs = [
    ...
    { title: 'Design a brand identity for a logistics startup', description: 'Create logo, colour palette, typography, and a basic brand guide for a last-mile delivery company based in Nairobi.', budget: 600, category: 'Design', client: client2Id },
    ...
    { title: 'Set up CI/CD pipeline on GitHub Actions', description: 'Configure GitHub Actions workflows for a Node.js API: lint, test, Docker build, and deploy to Railway on merge to main. Staging and production environments included.', budget: 500, category: 'DevOps', client: client2Id },
    ...
    { title: 'Build a Python data pipeline for sales analytics', description: 'ETL pipeline pulling data from Salesforce, transforming it, and loading into BigQuery. Daily scheduled runs via Airflow or Prefect.', budget: 2000, category: 'Data Engineering', client: client2Id },
    ...
    { title: 'Translate app UI strings — English to Swahili & Yoruba', description: 'Translate approximately 1500 UI strings for a mobile fintech app. Maintain tone consistency across both languages.', budget: 250, category: 'Translation', client: clientId },
  ...
  await run(
    ...
    [crypto.randomUUID(), jobIds[0], freelancer1, 'I have 5 years of React experience and have built 3 e-commerce storefronts.', 1400, 'pending'],
  )
  await run(
    'INSERT INTO proposals (id, job_id, freelancer_id, cover_letter, proposed_amount, status) VALUES (?, ?, ?, ?, ?, ?)',
